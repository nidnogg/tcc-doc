...
TODO Explicar mutation rounds, funcionamento de pool e como fuzzer se encaixaria

A estrutura básica do algoritmo genético:

1) Primeiro é gerada uma população aleatório de N cromossomos com L-bits. As populações são candidatos para a solução do problema.
2) É calculada a função fitness F(x) de cada cromossomo x na população.
3) As etapas seguinte (a,b e c) são repetidas até serem criadas N gerações:
a) Um par de cromossomos geradores da população atual são selecionados, com a probabilidade de seleção sendo uma função crescente fitness. O mesmo cromossomo pode ser selecionado mais de uma vez para ser gerador, processo chamado de "seleção com substituição".
b) É feito um crossover no par em um ponto aleatório escolhido com probabilidade uniforme para formar dois descendentes. Se nenhum crossover ocorrer, gere dois descendentes identicos aos pais.  
c) Execute a mutação nos dois descendentes com uma probabilidade de mutação em cada bit e coloque os cromossomos na nova população.
4) Substitua a população antiga com a nova.
5) Volte para etapa 2.

WAFAmole utiliza operadores de mutação específicos para o passo c:
Case Swapping
